...
class Main : XposedModule() {
    ...
    private fun hookLocationMethods(classLoader: ClassLoader) {
        ...
        hookAllMethods(locationClass, "isFromMockProvider") { chain ->
            if (isHoneypot(chain.thisObject)) chain.proceed() else false
        }
        ...
        hookAllMethods(locationClass, "setIsFromMockProvider") { chain ->
            if (isHoneypot(chain.thisObject)) {
                chain.proceed()
            } else {
        ...
        hookAllMethods(locationClass, "getExtras") { chain ->
            ...
            if (isHoneypot(chain.thisObject)) extras else getPatchedBundle(extras)
        }
        ...
        hookAllMethods(locationClass, "getProvider") { chain ->
            ...
            when {
                provider == null -> null
                isHoneypot(chain.thisObject) -> provider
                provider !in knownProviders -> "gps"
                else -> provider
            }
        }

        // Refleksi field untuk masking sistem internal
